...
namespace CafeShopManagementSystem
{
    public partial class CashierOrderForm : UserControl
    {
        ...
        public void displayTotalPrice()
        {
            ...
            if (connect.State == ConnectionState.Closed)
            {
                try
                {
                    ...
                    string selectDate = "SELECT SUM(prod_price) FROM orders WHERE customer_id = @custID";
        ...
        public void clearFields()
        {
            ...
            cashierOrderForm_prodName.Text = "_";
            cashierOrderForm_price.Text = "_";
            cashierOrderForm_orderPrice.Text = "_";
            cashierOrderForm_amount.Text = "";
            cashierOrderForm_change.Text = "0";

        }

        public void clearFields_add()
        {
            cashierOrderForm_type.SelectedIndex = -1;
            cashierOrderForm_productID.SelectedIndex = -1;
            cashierOrderForm_quantity.Value = 0;
        ...
        {
            using (SqlConnection connect = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=""D:\FPTU Subject\PRN212\Project\CafeShopManagementSystem\Database\cafe.mdf"";Integrated Security=True;Connect Timeout=30"))
            {
                connect.Open();
        ...
        private void cashierOrderForm_type_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (connect.State == ConnectionState.Closed)
            {
                ...
                {
                    MessageBox.Show("Failed Connction: " + ex, "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                finally
                {
                    connect.Close();
        ...
        private void cashierOrderForm_payBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Something went wrong. Please check the order details and amount.", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            // Validate that amount is sufficient
            if (!float.TryParse(cashierOrderForm_amount.Text, out float amount) || amount < totalPrice)
            {
                MessageBox.Show("The entered amount is less than the total price. Please enter a sufficient amount.", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                cashierOrderForm_amount.Text = "";
                return;
            }

            // Confirm payment
            if (MessageBox.Show("Are you sure you want to proceed with the payment?", "Confirmation Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                if (connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand(insertData, connect))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@change", amount - totalPrice);
        ...
        private void cashierOrderForm_receiptBtn_Click(object sender, EventArgs e)
        {
            ...
            if(MessageBox.Show("Are you sure you want to move to next Order?", "Confirmation Message"
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            string headerText = "FiveStar Cafe Shop";
