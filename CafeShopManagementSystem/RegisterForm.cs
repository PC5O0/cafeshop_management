                ...
                if(connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        using (SqlCommand checkUsername = new SqlCommand(selectUsername, connect))
                        {
                            ...
                            {
                                MessageBox.Show("Invalid Password (need atleast 8 characters)", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                            else
                            {
                                string inserDate = "INSERT INTO users (username, password, profile_image, role, status, date_reg)" +
                                    " VALUES (@usern, @pass, @image, @role, @status, @date)";
                                DateTime today = DateTime.Today;
                                using(SqlCommand cmd = new SqlCommand(inserDate, connect))
                                {
